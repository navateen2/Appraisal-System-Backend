...
@router.get(
    "", response_model=list[AppraisalResponse], dependencies=[Depends(require_role(UserRole.HR, UserRole.Employee))]
)
...
async def get_appraisal_by_id(appraisal_id: int, db: AsyncSession = Depends(get_db)):
    result = await service.get_appraisal_by_id(appraisal_id, db)
    return result


@router.delete(
    "/{appraisal_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role(UserRole.HR))]
)
...
async def get_appraisal_meeting_notes(appraisal_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {"meeting_notes": appraisal.hr_notes, "updated_at": appraisal.updated_at}
...
async def update_appraisal_status(
    ...
    updated_appraisal = await service.transition_appraisal_status(
        db=db, appraisal_id=appraisal_id, target_status=payload.status, current_user=current_user
    )

    return UpdateStatusResponse(message="Appraisal status updated successfully", status=updated_appraisal.status)
