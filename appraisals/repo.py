...
async def create(
    ...
    appraisal = Appraisal(cycle_id=cycle_id, employee_id=employee_id, status=AppraisalStatus.Initiated)
...
async def get_detailed_appraisal_by_id(db: AsyncSession, appraisal_id: int) -> Appraisal | None:
    stmt = (
        select(Appraisal)
        .options(selectinload(Appraisal.self_appraisal), selectinload(Appraisal.lead_assignments))
