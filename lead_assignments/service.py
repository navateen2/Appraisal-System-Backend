...
async def assign_leads_to_appraisal(
    appraisal_id: int, 
    lead_ids: List[int], 
    hr_id: int, 
    db: AsyncSession
    ...
    elif appraisal.status == AppraisalStatus.FeedbackInitiated:
        existing_leads = await repo.get_existing_assigned_lead_ids(appraisal_id, db)
...
async def submit_lead_feedback(mapping_id: int, current_user_id: int, db: AsyncSession):
    ...
    if assignment.lead_id != current_user_id:
        raise ForbiddenException(
            "Access denied: You are not authorized to submit feedback for this assignment."
        )
    ...
    if assignment.appraisal is None or assignment.appraisal.deleted_at is not None or assignment.appraisal.status!=AppraisalStatus.InitiateFeedback:
        raise NotFoundException("Operation rejected: The associated appraisal lifecycle does not exist or has been deleted or appraisal status not Initiate Feedback")
