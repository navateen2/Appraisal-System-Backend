...
from lead_assignments.schemas import (
    AssignLeadsRequest,
...
async def get_assigned_leads(appraisal_id: int, db: AsyncSession = Depends(get_db)):
    return await service.get_assigned_leads(appraisal_id, db)


@router.delete("/lead-assignment/{mapping_id}", response_model=str, dependencies=[Depends(require_role(UserRole.HR))])
async def remove_assigned_lead(mapping_id: int, db: AsyncSession = Depends(get_db)):
...
async def submit_lead_feedback(
    mapping_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(get_current_user)
):
...
async def get_my_pending_assignments(
    lead_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(get_current_user)
):
    return await service.get_pending_assignments_for_lead(lead_id, current_user.id, db)


@router.get(
    "/lead-assignment/lead/{lead_id}/all",
    response_model=List[LeadAssignmentResponse],
    dependencies=[Depends(require_role(UserRole.Employee))],
)
async def get_my_assignments(
    ...
    return await service.get_assignments_for_lead(lead_id, current_user.id, db)


@router.get(
    "/{mapping_id}/employee",
    response_model=EmployeeInfoResponse,
)
async def get_employee_by_mapping(
    mapping_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: TokenPayload = Depends(get_current_user),
):
    return await service.get_employee_by_mapping_id(
        db=db,
